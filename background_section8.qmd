...
The chunk below sets up a `reticulate` environment on-the-fly for use with this Quarto document. The environment is created in the current working directory, and will be named `OSTA`. The environment will be created with Python version 3.8.18, and the required packages will be installed.

```{r}
#| label: reticulate_env
